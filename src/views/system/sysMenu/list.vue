<template>
  <div class="app-container">
    ...
    <!-- 工具条 -->
    <div class="tools-div">
      <el-button type="success" icon="el-icon-plus" size="mini" @click="add()" :disabled="$hasBP('bnt.sysMenu.add')  === false">添 加 目 录</el-button>
    </div>
    <el-table
      :data="sysMenuList"
...
export default {
  ...
  data() {
    return {
      ...
      typeDisabled: false, //菜单类型是否可点击
      dialogTitle: '', //添加节点时的标题
  ...
  methods: {
    ...
    update() {
      //调用api中更新的方法
      api.update(this.sysMenu).then(res=>{
        this.$message.success(res.message);
        this.fetchData();
      });
    }
